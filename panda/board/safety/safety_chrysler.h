...
const CanMsg CHRYSLER_RAM_DT_TX_MSGS[] = {
  ...
  {CHRYSLER_RAM_DT_ADDRS.DAS_3, 0, 8},
};
...
static int chrysler_fwd_hook(int bus_num, CANPacket_t *to_fwd) {
  ...
  const bool is_lkas = ((addr == chrysler_addrs->LKAS_COMMAND) || (addr == chrysler_addrs->DAS_6) || (addr == chrysler_addrs->CRUISE_BUTTONS));// || (addr == chrysler_addrs->LKAS_HEARTBIT));
